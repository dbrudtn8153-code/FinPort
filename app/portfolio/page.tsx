...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    const savedCashTargetWeight = localStorage.getItem(
      "portfolio-cash-target-weight"
    );
    ...
    if (savedCashTargetWeight !== null) {
      setCashTargetWeight(savedCashTargetWeight);
    }
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem("portfolio-cash-amount", cashAmount);
  }, [cashAmount]);

  useEffect(() => {
    localStorage.setItem("portfolio-cash-target-weight", cashTargetWeight);
  }, [cashTargetWeight]);
  ...
  async function calculatePortfolio() {
    ...
    try {
      ...
      const emptySymbolItems = normalizedHoldings.filter((item) => !item.symbol);
      if (emptySymbolItems.length > 0) {
        setResults([]);
        setMessage("종목을 입력한 뒤 검색 결과를 클릭해서 선택해주세요.");
        setLoading(false);
        return;
      }

      const unselectedNameItems = normalizedHoldings.filter(
        (item) => !/^[A-Z0-9.\-]+$/.test(item.symbol)
      );

      if (unselectedNameItems.length > 0) {
        setResults([]);
        setMessage("종목명을 입력한 뒤 검색 결과를 클릭해서 선택해주세요.");
        setLoading(false);
        return;
      }

      const invalidNumberItems = normalizedHoldings.filter(
        (item) =>
          Number.isNaN(item.quantity) ||
          Number.isNaN(item.avgPrice) ||
          item.quantity <= 0 ||
          item.avgPrice <= 0
      );
      ...
      if (invalidNumberItems.length > 0) {
        setResults([]);
        setMessage("수량과 평균 매수가는 0보다 큰 숫자로 입력해주세요.");
        setLoading(false);
        return;
      }

      const invalidTargetWeightItems = normalizedHoldings.filter(
        (item) =>
          Number.isNaN(item.targetWeight) ||
          item.targetWeight < 0 ||
          item.targetWeight > 100
      );

      if (invalidTargetWeightItems.length > 0) {
        setResults([]);
        setMessage("종목 목표 비중은 0 이상 100 이하 숫자로 입력해주세요.");
        setLoading(false);
        return;
      }
      ...
      if (Number.isNaN(cashValue) || cashValue < 0) {
        setResults([]);
        setMessage("현금 금액은 0 이상의 숫자로 입력해주세요.");
        setLoading(false);
        return;
      }
      ...
      if (Number.isNaN(cashTarget) || cashTarget < 0 || cashTarget > 100) {
        setResults([]);
        setMessage("현금 목표 비중은 0 이상 100 이하 숫자로 입력해주세요.");
        setLoading(false);
        return;
      }
      ...
      const duplicateSymbols = Object.keys(symbolCounts).filter(
        (symbol) => symbolCounts[symbol] > 1
      );

      if (duplicateSymbols.length > 0) {
        setResults([]);
        setMessage(`중복된 티커가 있습니다: ${duplicateSymbols.join(", ")}`);
        setLoading(false);
        return;
      }

      const totalTargetWeight =
        normalizedHoldings.reduce((sum, item) => sum + item.targetWeight, 0) +
        cashTarget;

      if (Math.round(totalTargetWeight * 10) / 10 !== 100) {
        setResults([]);
        setMessage(`목표 비중 합계가 100%가 아닙니다. 현재 합계: ${totalTargetWeight}%`);
        setLoading(false);
        return;
      }

      const validHoldings = normalizedHoldings;

      const symbols = validHoldings.map((item) => item.symbol).join(",");
      const res = await fetch(`/api/etf?symbols=${symbols}`);
      const json = await res.json();

      if (!json.results || json.results.length === 0) {
        setResults([]);
        setMessage("현재가 데이터를 불러오지 못했습니다.");
        setLoading(false);
        return;
      }

      const priceMap = new Map<string, PriceItem>();
      json.results.forEach((item: PriceItem) => {
        priceMap.set(item.symbol, item);
      });

      const calculated: HoldingResult[] = validHoldings.map((item) => {
        ...
        const costBasisKrw =
          currency === "USD" ? costBasis * usdToKrw : costBasis;
        const marketValueKrw =
          currency === "USD" ? marketValue * usdToKrw : marketValue;
      ...
      const missingSymbols = validHoldings
        .filter((item) => !priceMap.has(item.symbol))
        .map((item) => item.symbol);

      if (missingSymbols.length > 0) {
        setMessage(
          `현재가를 찾지 못한 티커가 있습니다: ${missingSymbols.join(", ")}`
        );
      }
    } catch {
      setResults([]);
      setMessage("포트폴리오 계산 중 오류가 발생했습니다.");
    } finally {
      setLoading(false);
    }
  }

  const cashValueKrw = useMemo(() => {
    const value = Number(cashAmount);
    return Number.isNaN(value) ? 0 : value;
  }, [cashAmount]);

  const summary = useMemo(() => {
    const holdingsCostBasisKrw = results.reduce(
      (sum, item) => sum + item.costBasisKrw,
      0
    );
    const holdingsMarketValueKrw = results.reduce(
      (sum, item) => sum + item.marketValueKrw,
      0
    );
    ...
    const totalReturnRate =
      totalCostBasisKrw > 0
        ? (totalProfitLossKrw / totalCostBasisKrw) * 100
        : 0;
  ...
  const totalTargetWeight = useMemo(() => {
    return (
      holdings.reduce((sum, item) => sum + Number(item.targetWeight || 0), 0) +
      Number(cashTargetWeight || 0)
    );
  ...
  const weights = useMemo(() => {
    const holdingWeights = results.map((item) => {
      const weight =
        summary.totalMarketValueKrw > 0
          ? (item.marketValueKrw / summary.totalMarketValueKrw) * 100
          : 0;
    ...
    if (summary.cashValueKrw > 0) {
      holdingWeights.push({
        ...
        weight:
          summary.totalMarketValueKrw > 0
            ? (summary.cashValueKrw / summary.totalMarketValueKrw) * 100
            : 0,
      });
    }
  ...
  const recommendation = useMemo(() => {
    ...
    if (loss < -20) {
      return "현재 큰 손실 상태입니다. 비중 문제보다 진입 시점이나 시장 상황 점검이 필요합니다.";
    }

    if (loss < -10) {
      return "손실이 발생하고 있습니다. 포트폴리오 구조 또는 투자 시점을 점검해보세요.";
    }

    if (cash >= 30) {
      return "현금 비중이 높아 방어적인 포트폴리오입니다. 추가 매수 기회를 기다리는 전략에 가깝습니다.";
    }

    if (qqq > 50) {
      return "QQQ 비중이 매우 높아 변동성이 큰 공격적인 포트폴리오입니다.";
    }

    if (voo > 50) {
      return "VOO 중심의 안정적인 포트폴리오입니다. 장기 투자에 적합합니다.";
    }

    if (weights.length === 1) {
      return "단일 자산에 집중되어 있습니다. 분산을 고려해보세요.";
    }
  ...
  const rebalanceSuggestions = useMemo<RebalanceSuggestion[]>(() => {
    ...
    const stockSuggestions: RebalanceSuggestion[] = results
      .map((item) => {
        const holding = holdings.find(
          (h) => h.symbol.trim().toUpperCase() === item.symbol
        );
        ...
        const targetValueKrw =
          (summary.totalMarketValueKrw * targetWeight) / 100;

        const diffKrw = targetValueKrw - item.marketValueKrw;

        const priceKrw =
          item.currency === "USD"
            ? item.currentPrice * usdToKrw
            : item.currentPrice;
        ...
        const shares =
          item.currency === "KRW"
            ...
            : Number(rawShares.toFixed(2));

        const shareUnitText =
          item.currency === "KRW"
            ? `${Math.abs(shares).toLocaleString()}주`
            : `${Math.abs(shares).toFixed(2)}주`;
    ...
    if (cashTarget > 0) {
      const currentCashWeight =
        summary.totalMarketValueKrw > 0
          ? (summary.cashValueKrw / summary.totalMarketValueKrw) * 100
          : 0;

      const targetCashValueKrw =
        (summary.totalMarketValueKrw * cashTarget) / 100;
      ...
      stockSuggestions.push({
        symbol: "CASH",
        name: "현금",
        currentWeight: currentCashWeight,
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="rounded-3xl bg-gradient-to-br from-slate-800 to-slate-950 px-8 py-12 text-white shadow-xl md:px-12">
          <div className="max-w-3xl">
            <div className="mb-4 inline-flex rounded-full bg-white/10 px-4 py-1 text-sm">
            ...
            <p className="mt-4 text-base text-slate-300 md:text-lg">
              종목명 검색, 목표 비중, 현금 비중까지 반영해 실전형 리밸런싱을 계산합니다.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-slate-500">
                종목명 또는 티커, 수량, 평균 매수가, 목표 비중을 입력하세요.
              </p>
            </div>
          ...
          <div className="space-y-4">
            {holdings.map((item) => (
              <div
                key={item.id}
                className="grid items-center gap-3 rounded-2xl border border-slate-200 p-4 md:grid-cols-[1fr_1fr_1fr_1fr_auto]"
              >
                <div>
                  <label className="mb-2 block text-sm font-medium text-slate-600 md:hidden">
                    종목
                  </label>
                  <SearchInput item={item} updateHolding={updateHolding} />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={item.quantity}
                    onChange={(e) =>
                      updateHolding(item.id, "quantity", e.target.value)
                    }
                    placeholder="예: 10"
                    className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    value={item.avgPrice}
                    onChange={(e) =>
                      updateHolding(item.id, "avgPrice", e.target.value)
                    }
                    placeholder="예: 500 또는 210000"
                    className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateHolding(item.id, "targetWeight", e.target.value)
                    }
                    placeholder="예: 25"
                    className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500"
                  />
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="1"
                  value={cashTargetWeight}
                  onChange={(e) => setCashTargetWeight(e.target.value)}
                  placeholder="예: 20"
                  className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500"
                />
              </div>
          ...
          {message && (
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-700">
              {message}
            </div>
          )}
        </div>
        ...
        <>
          <section className="mx-auto max-w-6xl px-6 py-8">
            <div className="grid gap-4 md:grid-cols-4">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="text-sm text-slate-500">총 투자원금 (KRW)</div>
                <div className="mt-2 text-3xl font-bold">
                  {formatCurrency(summary.totalCostBasisKrw, "KRW")}
                </div>
              </div>

              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="text-sm text-slate-500">총 평가금액 (KRW)</div>
                <div className="mt-2 text-3xl font-bold">
                  {formatCurrency(summary.totalMarketValueKrw, "KRW")}
                </div>
              </div>
            ...
            <div className="mt-6 rounded-3xl border border-blue-200 bg-blue-50 p-6 shadow-sm">
              ...
              <div className="mt-2 text-slate-700">
                현재 포트폴리오는 <span className="font-semibold">{style}</span> 입니다.
              </div>
              ...
              <div className="mt-6 h-64 w-full">
                <ResponsiveContainer>
                  <PieChart>
                    ...
                    >
                      <Label
                        value={formatCurrency(summary.totalMarketValueKrw, "KRW")}
                        position="center"
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="text-sm text-slate-600">
                            현재 {item.currentWeight.toFixed(1)}% → 목표 {item.targetWeight}%
                          </div>

                          {item.isCash ? (
                            <div
                              className={`mt-1 text-sm font-semibold ${item.diffKrw >= 0 ? "text-emerald-600" : "text-red-600"
                                }`}
                            >
                              {item.diffKrw >= 0 ? "현금 비중 확대" : "현금 투입"}{" "}
                              {formatCurrency(Math.abs(item.diffKrw), "KRW")}
                            </div>
                          ) : (
                            <div
                              className={`mt-1 text-sm font-semibold ${item.diffKrw >= 0 ? "text-emerald-600" : "text-red-600"
              ...
              <p className="mt-4 text-sm text-slate-500">
                적용 환율: 1 USD = ₩{usdToKrw.toLocaleString()}
              </p>
            </div>
          </section>
          ...
          <section className="mx-auto max-w-6xl px-6 pb-16">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-6">
                <h3 className="text-2xl font-bold">보유 종목 상세</h3>
                <p className="mt-1 text-slate-500">
                  개별 종목은 원래 통화로 표시하고, 총합은 KRW 기준으로 계산합니다.
                </p>
              </div>
              ...
              <div className="mb-4 rounded-2xl bg-slate-50 px-4 py-4">
                ...
                <div className="mt-2 text-xl font-bold">
                  {formatCurrency(summary.cashValueKrw, "KRW")}
                </div>
              </div>
                ...
                <table className="min-w-full border-separate border-spacing-y-3">
                  <thead>
                    <tr className="text-left text-sm text-slate-500">
                      <th className="px-4">종목</th>
                      <th className="px-4">보유 수량</th>
                      <th className="px-4">평균 매수가</th>
                      <th className="px-4">현재가</th>
                      <th className="px-4">투자원금</th>
                  ...
                  <tbody>
                    {results.map((item) => (
                      <tr key={item.id} className="rounded-2xl bg-slate-50">
                        ...
                        <td
                          className={`px-4 py-4 font-semibold ${item.profitLoss >= 0 ? "text-emerald-600" : "text-red-600"
                            }`}
                        >
                          {formatCurrency(item.profitLoss, item.currency)}
                        </td>
                        <td
                          className={`rounded-r-2xl px-4 py-4 font-semibold ${item.returnRate >= 0 ? "text-emerald-600" : "text-red-600"
                            }`}
                        >
